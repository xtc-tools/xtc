...
# Create meshes
sch.define_memory_mesh(axes=["mx", "my"], sizes=[2, 2])
sch.define_processor_mesh(axes=["px", "py", "psx", "psy"], sizes=[2, 2, 2, 8])
sch.tile("i", {"i1": 2})
sch.tile("j", {"j1": 16})
...
# CHECK-NEXT:      %6 = transform.sdist.distribute_loop %forall_op {axis = "./i", mesh = "processor_mesh"} : (!transform.any_op) -> !transform.any_op
# CHECK-NEXT:      transform.yield
# CHECK-NEXT:    }
# CHECK-NEXT:  }
# CHECK-NEXT:  
...
# CHECK-NEXT:        sdist.for_distributed %arg4 = %c0_5 to %c2 step [%c1_6] on @processor_mesh("./i") {
