...
# CHECK-NEXT:      %0 = transform.structured.match attributes {sym_name = "matmul_relu"} in %arg0 : (!transform.any_op) -> !transform.any_op
...
# CHECK-NEXT:      } : !transform.any_op
# CHECK-NEXT:      %4 = transform.structured.match attributes {__xtc_id_relu_} in %arg0 : (!transform.any_op) -> !transform.any_op
# CHECK-NEXT:      %tiled_linalg_op_6, %loops_7 = transform.structured.tile_using_for %4 tile_sizes [1] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
...
# CHECK-NEXT:  #map4 = affine_map<(d0, d1) -> ()>
# CHECK-NEXT:  module attributes {transform.with_named_sequence} {
# CHECK-NEXT:    func.func @matmul_relu(%arg0: tensor<4x512xf32> {llvm.noalias}, %arg1: tensor<512x32xf32> {llvm.noalias}, %arg2: memref<4x32xf32> {llvm.noalias}) {
...
# CHECK-NEXT:      %2 = linalg.fill {__xtc_id_matmul_0_} ins(%cst : f32) outs(%1 : tensor<4x32xf32>) -> tensor<4x32xf32>
...
# CHECK-NEXT:      %5 = scf.for %arg3 = %c0_0 to %c4_1 step %c1_2 iter_args(%arg4 = %4) -> (tensor<4x32xf32>) {
...
# CHECK-NEXT:        %6 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel"]} ins(%extracted_slice, %cst : tensor<1x32xf32>, f32) outs(%extracted_slice_3 : tensor<1x32xf32>) attrs =  {__xtc_id_relu_} {
...
# CHECK-NEXT:      %1 = scf.for %arg3 = %c0 to %c512 step %c1 iter_args(%arg4 = %alloca) -> (memref<4x32xf32>) {
...
# CHECK-NEXT:      } {"./k"}
# CHECK-NEXT:      %2 = scf.for %arg3 = %c0 to %c4 step %c1 iter_args(%arg4 = %arg2) -> (memref<4x32xf32>) {
# CHECK-NEXT:        %subview = memref.subview %1[%arg3, 0] [1, 32] [1, 1] : memref<4x32xf32> to memref<1x32xf32, strided<[32, 1], offset: ?>>
# CHECK-NEXT:        %subview_1 = memref.subview %arg4[%arg3, 0] [1, 32] [1, 1] : memref<4x32xf32> to memref<1x32xf32, strided<[32, 1], offset: ?>>
# CHECK-NEXT:        linalg.generic {indexing_maps = [#map, #map1, #map], iterator_types = ["parallel", "parallel"]} ins(%subview, %cst_0 : memref<1x32xf32, strided<[32, 1], offset: ?>>, f32) outs(%subview_1 : memref<1x32xf32, strided<[32, 1], offset: ?>>) attrs =  {__xtc_id_relu_} {
# CHECK-NEXT:        ^bb0(%in: f32, %in_3: f32, %out: f32):
# CHECK-NEXT:          %3 = arith.maximumf %in, %in_3 : f32
...
# CHECK-NEXT:        memref.copy %subview_1, %subview_2 : memref<1x32xf32, strided<[32, 1], offset: ?>> to memref<1x32xf32, strided<[32, 1], offset: ?>>
