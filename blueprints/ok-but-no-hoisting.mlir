...
module attributes {transform.with_named_sequence} {
  ...
  transform.named_sequence @seq1(%arg0: !transform.any_op {transform.consumed}) {
    ...
    %tiled_linalg_op, %loops = transform.structured.tile_using_for %tiled_op[0, 0, 8] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.annotate %loops "k" : !transform.any_op
    %tiled_linalg_op_0, %loops_1 = transform.structured.tile_using_for %tiled_linalg_op[0, 8, 0] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.annotate %loops_1 "j" : !transform.any_op
    ...
    %2 = transform.structured.hoist_redundant_vector_transfers %1 : (!transform.any_op) -> !transform.any_op
    transform.apply_patterns to %2 {
      transform.apply_patterns.vector.lower_outerproduct
      transform.apply_patterns.vector.lower_contraction
      transform.apply_patterns.canonicalization
    } {apply_cse} : !transform.any_op
    %3 = transform.structured.match attributes {j1} in %2 : (!transform.any_op) -> !transform.any_op
    transform.loop.unroll %3 {factor = 8 : i64} : !transform.any_op
    %4 = transform.structured.match attributes {i1} in %2 : (!transform.any_op) -> !transform.any_op
    transform.loop.unroll %4 {factor = 8 : i64} : !transform.any_op
    transform.yield 
  }
