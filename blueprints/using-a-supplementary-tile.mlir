...
module attributes {transform.with_named_sequence} {
  ...
  // func.func @main() {
  //   %cst = arith.constant 0.819450676 : f32
  //   %splat = tensor.splat %cst : tensor<512x1024xf32>
  //   %cst_0 = arith.constant 0.67076534 : f32
  ...
  transform.named_sequence @seq1(%arg0: !transform.any_op {transform.consumed}) {
    // %g = transform.structured.generalize %arg0 : (!transform.any_op) -> !transform.any_op
    
    %tiled_op, %forall_op = transform.structured.tile_using_forall %arg0 tile_sizes [4, 0, 0] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.annotate %forall_op "i" : !transform.any_op
    %tiled_linalg_op, %loops = transform.structured.tile_using_for %tiled_op[0, 16, 0] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    %tiled_linalg_op_2, %i1 = transform.structured.tile_using_for %tiled_linalg_op_0[1, 0, 0] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.annotate %i1 "i1" : !transform.any_op
    %tiled_linalg_op_4, %j1 = transform.structured.tile_using_for %tiled_linalg_op_2[0, 8, 0] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.annotate %j1 "j1" : !transform.any_op
    ...
    %2 = transform.structured.match attributes {j1} in %1 : (!transform.any_op) -> !transform.any_op
    // transform.loop.unroll %2 {factor = 8 : i64} : !transform.any_op
    transform.loop.unroll %2 {factor = 2 : i64} : !transform.any_op
    %3 = transform.structured.match attributes {i1} in %1 : (!transform.any_op) -> !transform.any_op
    transform.loop.unroll %3 {factor = 4 : i64} : !transform.any_op
    %4 = transform.apply_registered_pass "lower-affine" to %1 : (!transform.any_op) -> !transform.any_op
    %5 = transform.apply_registered_pass "sccp" to %4 : (!transform.any_op) -> !transform.any_op
    %6 = transform.apply_registered_pass "loop-invariant-code-motion" to %5 : (!transform.any_op) -> !transform.any_op
    transform.apply_patterns to %6 {
      transform.apply_patterns.canonicalization
    } {apply_cse} : !transform.any_op
    %7 = transform.structured.hoist_redundant_vector_transfers %6 : (!transform.any_op) -> !transform.any_op
    transform.yield 
  }
