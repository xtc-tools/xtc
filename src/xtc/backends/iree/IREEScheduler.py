...
# associated tile size (e.g. an untiled base dimension). This defaults to IREE's
# native vector size for f32 (512-bit / 4 bytes = 16 elements); IREE narrows the
# vectors itself when the actual target is smaller, so a larger default is safe.
_VECTOR_WIDTH = 16
