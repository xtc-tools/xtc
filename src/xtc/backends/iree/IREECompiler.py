...
class IREECompiler(itf.comp.Compiler):
    ...
    def compile(self, schedule: itf.schd.Schedule) -> itf.comp.Module:
        ...
        # An anonymous temp vmfb is owned by the returned module (deleted with
        # it); an explicit dump_file is the caller's to keep.
        owns_file = dump_file is None
        if dump_file is None:
            fd, vmfb_path = tempfile.mkstemp(prefix=f"{name}_", suffix=".vmfb")
            os.close(fd)
        else:
