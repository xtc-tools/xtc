...
class MlirProgramInsertTransformPass:
    ...
    def _create_sdist_meshes(self, schedule: MlirNodeSchedule) -> None:
        ...
        with (
            ...
            if len(schedule.memory_mesh[0]) > 0:
                mesh = [
                    {axis_name: axis_size}
                    for axis_name, axis_size in zip(
                        schedule.memory_mesh[0], schedule.memory_mesh[1]
                    )
                ]
            ...
            if len(schedule.processor_mesh[0]) > 0:
                mesh = [
                    {axis_name: axis_size}
                    for axis_name, axis_size in zip(
                        schedule.processor_mesh[0], schedule.processor_mesh[1]
                    )
                ]
                assert sdist_transform is not None
    ...
    def _generate_node_scheduling(
        ...
        for loop_name in permutation:
            ...
            if loop_name in schedule.local_buffers.keys():
                self._local_buffer(
                    loop_name=loop_name,
                    schedule=schedule,
                    sched_state=sched_state,
    ...
    def _local_buffer(
        ...
        for input_idx in schedule.local_buffers[loop_name]:
            assert sdist_transform is not None
