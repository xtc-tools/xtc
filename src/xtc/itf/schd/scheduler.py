...
class Scheduler(ABC):
    ...
    def define_memory_mesh(self, axes: list[str], sizes: list[int]) -> None:
        ...
        Args:
            axes: axes names to define the memory mesh
            sizes: sizes of the memory mesh
        """
        ...
    ...
    def define_processor_mesh(self, axes: list[str], sizes: list[int]) -> None:
        ...
        Args:
            axes: axes names to define the processor mesh
            sizes: sizes of the processor mesh
        """
        ...
    ...
    def local_buffer_at(
        ...
        """Create a local buffer at a given level for a specific input index.

        This method creates a local buffer for the given input buffer index
        at the specified axis level. The source buffer must be distributed.

        Args:
            axis: the axis level where the local buffer should be created
            input_idx: input buffer index for the scheduled computation
            root: the parent split (or the operator's absolute root)
        """
        ...
